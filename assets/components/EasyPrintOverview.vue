      ...
      >
        <div class="bg-white dark:bg-stone-900 rounded-lg shadow-lg max-w-sm w-full p-6 text-center">
          <h3 class="text-lg font-semibold mb-4 dark:text-stone-200">
            Möchten Sie die Liste erstellen?
          </h3>
          <div class="flex justify-center gap-4">
            <button
              @click="confirmPrintList"
...
onMounted(async () => {
  ...
  try {
    // STEP 1: Grunddaten
    const { data } = await axios.get('/api/printorders');
    bump(1);
...
async function confirmPrintList() {
  ...
  try {
    ...
    if (data?.success) {
      toast.success(`Liste #${data.id} erstellt (${data.count} Positionen).`)
      window.open(`/api/printlist/${data.id}/pdf`, '_blank')
      // ggf. Tabelle neu laden
      await reloadData()
    } else {
